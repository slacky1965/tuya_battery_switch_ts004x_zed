...
Если на пульте не нажимать кнопки, то он уснет на время, указанное в параметре `Max rep interval`. Т.е. исходя из скриншота - на 14400 секунд, что соответствует 4-м часам. По истечению этого времени пульт простнется, измерит батарейку, вышлет репорт и опять уснет на это же время.
...
Для того, чтобы все работало, можно обойтись без настройки биндинга. Создаем группу с каким-нибудь именем и присваиваем ей номер, допустим `10`. Номер или `ID` этой группы также является ее сетевым групповым адресом. Теперь в эту группу добавляем `Endpoint` лампочки и `Endpoint 2`, `Endpoint 3` пульта, где мы как раз настройли управление светом.
