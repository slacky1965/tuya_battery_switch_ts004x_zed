...
- `Switch type (Endpoint: X)` - выбор действия при нажатии на кнопку.
	...
	- `brightness_level_up` (**недоступно для однокнопочного пульта**) - устанавливает режим работы кнопки для увеличения яркости.
		...
		- Удержание - посылает команду `Move to level`. Устройство должно плавно увеличивать яркость, пока последняя не станет максимальной - значение максимальной яркости берется из атрибута `maxLevel`. Увеличение продолжается, пока удерживается кнопка и не достигнуто зачение `maxLevel`. Скорость увеличения яркости берется из атрибута `onOffTransitionTime`, который можно изменить через dev-консоль. По умолчанию это значение равно 25. Если устройство было выключено, оно включается.
